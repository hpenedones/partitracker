...
int main(int argc, char** argv)
{
	...
	CvRect object_region = cvRect( 	conf->object_reg_x,	conf->object_reg_y,
									conf->object_reg_width,	conf->object_reg_height);
	
	cvNamedWindow("Video");

	CvVideoWriter *writer;

	writer = cvCreateVideoWriter(conf->output_video_path.c_str() ,CV_FOURCC('D', 'I', 'V', 'X'), conf->output_fps, cvGetSize(img), 1);
	...
	for(int nframe=1; nframe<numFrames-1; nframe++)
		{			
			if (conf->visualize || conf->save_results)
			{
				...
				cvRectangle(img, cvPoint(estimate.x, estimate.y), cvPoint(estimate.x + estimate.width, estimate.y + estimate.height), cvScalar(0,255,0), 2);
